...
export default function ProductPage() {
  ...
  const fetchFavorites = async (userId) => {
    try {
      const response = await fetch(`https://serverhame-tg6x.onrender.com/api/favorites/${userId}`);
  ...
  useEffect(() => {
    if (userId) {
      const fetchCartItems = async () => {
        try {
          const response = await fetch(`https://serverhame-tg6x.onrender.com/api/cart?userId=${userId}`);
          const data = await response.json();
  ...
  const addToCart = async (product) => {
    ...
    try {
        await fetch('https://serverhame-tg6x.onrender.com/api/cart', {
            method: 'POST',
            headers: {
